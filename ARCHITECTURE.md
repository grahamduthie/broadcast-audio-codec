...
**Process restart persistence — fixed 2026-09-11.** The desired-link record now includes the Bleep/PFL intent and the pre-PFL Music volume. A Bleep event updates it atomically, and a new `GoXLRInterface` receives the saved values before its first `start()`. Thus an active codec link restored after `systemctl restart briclite` also reasserts PFL when it had been on. An explicit disconnect removes the complete record, including PFL intent.
