...
**This `start()` is the only source of truth for GoXLR config — not any saved `.goxlr` profile file (discovered 2026-09-09).** It runs from `PipelineController.start()` (`core/pipeline_manager.py:196`), which only executes when the pipeline actually starts: `/api/connect`, `_full_reconnect()` (channel-mode switch or Behringer hotplug), or the RX watchdog's auto-reconnect. It is **not** called merely by `briclite.service` starting or by `main.py`'s `lifespan()` — process boot only constructs the `PipelineController` object without starting it. So immediately after a host reboot or `systemctl restart`, before anything has connected, the physical GoXLR still shows whatever the `goxlr-utility` daemon's on-disk profile last had — not the fader layout/routing/colours documented below. On the PSA300 the daemon always loads a profile literally named `Default` (confirmed unchanged since 1 Jun across every boot), which has a *different* fader mapping and Music at volume 0 — this is expected pre-`start()` state, not a fault, and not something to "fix" by loading a different saved profile. Calling `POST /api/connect` (or triggering any reconnect) reapplies the correct config from code. Verify live state any time with `goxlr-client --status-json`.
...
**Still open:** this fix cannot and does not survive an actual `systemctl restart` of `briclite.service` (e.g. every code deploy) — that kills the whole Python process, wiping all in-memory state including a fresh `GoXLRInterface`, so PFL will still come up off after any deploy, same as before. Fixing that would need PFL desired-state persisted somewhere outside the process (e.g. read back from the GoXLR daemon's actual current routing on `start()`, rather than assumed) — not built.
