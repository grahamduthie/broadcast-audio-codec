...
**Pipeline lifecycle changed 2026-09-14 — TX capture now runs continuously, independent of the network link.** Until then, this whole pipeline (and the matching RX playback pipeline) was only set to `PLAYING` when the RTP link connected, and torn down again on disconnect — so the Broadcast Mix meter on the dashboard only showed real levels while connected. `PipelineController.begin_local()` now brings both TX and RX up once at process startup (and again after any full pipeline rebuild) and leaves them running regardless of connection status; `PipelineController.start()`/`stop()` now only gate the RTP socket, `_rx_loop`/`_playout_loop` threads, and whether `_on_tx_sample()` actually transmits — see `core/pipeline_manager.py`'s `begin_local()`/`start()`/`stop()`/`full_stop()` docstrings and `CURRENT-STATUS.md`'s 2026-09-14 entry for the full rationale (the GoXLR TX/RX-duplex constraint below is why this couldn't be a narrower "metering-only" tap). `full_stop()` (full teardown of both directions) is reserved for cases where the whole `PipelineController` object is being discarded and replaced — mode changes, hotplug, fault recovery, process shutdown. Confirmed live 2026-09-14: TX reaches `Gst.State.PLAYING` almost instantly at boot (continuous real ALSA data), while RX legitimately sits at `PAUSED` until the first real RTP buffer arrives via `/api/connect` — expected GStreamer preroll behaviour, not a fault; see `TROUBLESHOOTING.md`'s "journal shows RX: null→ready→paused but never paused→playing" entry before assuming otherwise.
...
**GoXLR interface uses a full reconnect instead** (`main.py`'s `_full_reconnect()`): fully tear down (`full_stop()`), build a brand-new `PipelineController` (passing the target `rx_channel_mode` into its constructor so the right pipeline is built from the start), bring the local pipeline back up (`begin_local()`), then restore the network link (`start()`) only if it was up before the rebuild. Discovered 2026-09-09: the GoXLR's TX (capture) and RX (playback) are two directions of the *same* USB audio device, and tearing down/reopening only the RX side in place while TX stays open reliably wedges the device (deterministic — reproduced with a single isolated mode switch, not just rapid clicking). A full stop/start cycles both directions together and has proven reliable under repeated stress-testing. This same full-reconnect path is also used for the Behringer hotplug rebuild (see §9) and for automatic recovery from a device-level pipeline fault. As of 2026-09-14, `_full_reconnect()` always rebuilds the local pipeline — even while the network link is disconnected, since the local pipeline now runs continuously (see §1's "Pipeline lifecycle changed 2026-09-14" note) — and only restores the network link afterward if it was active going in. The RX watchdog's auto-reconnect (§9/troubleshooting) also preserves the current channel mode across a reconnect. It used to also silently reset other GoXLR-specific state (Bleep/PFL) on every such reconnect — fixed 2026-09-09, see Known Limitations below.
...
- **Broadcast Mix (Master)** — the old TX Input meter: the post-everything combined on-air signal, with the -18 dBFS alignment line. As of 2026-09-14 this shows real levels regardless of RTP connection status (see §1's "Pipeline lifecycle changed 2026-09-14" note) — unlike Incoming Network below, which still requires an actual network link since it has no local source.
...
- Cough LED: cyan when disarmed, orange when armed with Line Out currently live (`_PFL_COLOUR` — changed 2026-09-14 from a dedicated green so this "feature engaged" indication matches Bleep's own PFL-on colour), red when armed and actively cutting.
...
**Channel mute buttons (`Fader1Mute`–`Fader4Mute`, one per A–D — added 2026-09-14):** unlike Bleep/Cough, these are each fader's own real hardware mute button (`SetFaderMuteFunction: "All"`, native Muted/Unmuted state genuinely toggled by the firmware — see the fader-status/button_down WebSocket events in `GOXLR-MINI-LINUX.md` §8). `_set_mute_button_colour()` in `goxlr.py` derives the LED from two pieces of state it now tracks for all four faders (`_fader_muted`, `_fader_volumes` — previously this tracking only existed for Fader C/Game and for Mic/LineIn respectively):
- Muted → red, full brightness, checked first so it wins regardless of fader position.
- Unmuted and the fader's own volume reads above `_MIC_OPEN_THRESHOLD` (the same threshold Studio Monitor Cut uses, reused here for all four faders even though Studio Monitor Cut itself only reacts to Mic/LineIn) → green, full brightness.
- Unmuted and closed → cyan, dim (the pre-existing default appearance, now made explicit and reasserted dynamically rather than left to native/profile defaults).

Full brightness in the "off" (Unmuted) state needs the same `SetButtonOffStyle: "Colour2"` trick as Bleep/Cough above; since these buttons need to look *different* while off depending on fader position (bright green vs. dim cyan), `_set_mute_button_colour()` toggles between `"Colour2"` and `"Dimmed"` every time open/closed state changes, rather than setting it once at start like Bleep/Cough do. The "on" (genuinely Muted) appearance was found live to show colour_one at full brightness regardless of `off_style` — confirmed by pressing mute on both an open and a closed fader and seeing the same full-bright red in both cases; this hadn't been exercised before since Bleep/Cough never actually enter their native Muted state. Any fader's `button_down` now gives an instant LED response (previously only Fader C/Game had this, for the clean-news-mute path); the authoritative `mute_state` patch still corrects it on release.

**Boot-restore bug, found and fixed 2026-09-14: `_fader_volumes` didn't reflect the just-restored levels.** `start()` seeds `_fader_volumes` from a `GetStatus` snapshot taken *before* the persisted levels are pushed to hardware (so it reflects whatever the daemon's on-disk profile happened to have), then `_apply_fader_volumes()` sends the correct `SetVolume` restore commands but never wrote the result back into `_fader_volumes` — only `_game_level` got that treatment. The only other way it could self-correct was a live WebSocket echo of those same `SetVolume` calls, which is unreliable at boot since the PFL monitor task connects only after `start()` returns. Since both the mute-button LEDs above and Studio Monitor Cut's `_mute_open()`/`_any_mic_open()` (§ Cough button, above) read `_fader_volumes` directly, a reboot with Mic/LineIn/Game genuinely closed could show all four fader buttons green and Studio Monitor Cut actively cutting Line Out with nothing open — reproduced live twice. Fix: `_apply_fader_volumes()` now updates `self._fader_volumes[channel]` immediately alongside each `SetVolume` push, the same way `_game_level` already did. Confirmed live 2026-09-14: after deploying the fix, a reboot with Mic/LineIn/Game closed and Console open showed correct LED colours and Cough at orange (armed, not cutting) immediately, no fader nudge required. Commit `370fdee`.
...
5. ~~**GoXLR Bleep/PFL state persisted across a full service restart — NOT IMPLEMENTED, open.**~~ **STALE — this was actually fixed by the 2026-09-11/13 restart-persistence work below and this item was never updated to say so.** Confirmed live 2026-09-14 across two independent full PSA300 reboots (not just `systemctl restart`) with PFL engaged beforehand: Bleep came back orange and `router.Music.Headphones` came back `true` both times, matching the persisted `studio_pfl: true` in `desired-link.json`. See "Process restart persistence" above — the record has carried Bleep/PFL intent since 2026-09-11, and a fresh `GoXLRInterface` reapplies it in `start()` the same way it does fader volumes/mic gain/studio-return level. If this ever regresses, suspect a change to that restore path rather than assuming it was never implemented.
