...
## Issue: journal shows "RX: null→ready→paused" but never "paused→playing" after a boot or rebuild

**Expected as of 2026-09-14, not a fault — confirmed live.** Since `PipelineController.begin_local()` now brings TX and RX up together at every boot/rebuild (see `CURRENT-STATUS.md`'s 2026-09-14 entry), you'll see `TX: paused→playing` complete almost instantly (its ALSA source has continuous real data from the moment it opens) but RX stop at `paused` and stay there indefinitely while disconnected. This is normal GStreamer behaviour, not a stall: RX's `rx_src` appsrc has nothing pushed into it until real RTP data arrives (`_playout_loop` only runs once `/api/connect` brings the network link up), so the pipeline legitimately sits in an async/preroll-pending state — GStreamer won't post the final `paused→playing` bus message until the sink actually receives a first buffer. The instant you connect and real RTP starts flowing, `RX: paused→playing` appears in the journal and Incoming Network starts showing real levels — confirmed live 2026-09-14. Don't chase this as a bug; only worry if RX is *still* stuck at `paused` a few seconds *after* a successful connect.
...
## Issue: after a reboot, Line Out is muted (Cough red) and fader mute-button LEDs show green even though nothing is open — FIXED 2026-09-14

**Root cause:** `GoXLRInterface.start()` seeded `_fader_volumes` from a `GetStatus` snapshot taken *before* the persisted fader levels were pushed to hardware — so it reflected the daemon's stale on-disk profile, not the actual restored levels. `_apply_fader_volumes()` sent the correct `SetVolume` commands to the device but never updated `_fader_volumes` itself; the only other way it could self-correct (a live WebSocket echo of those commands) is unreliable at boot since the PFL monitor task connects only after `start()` returns. Both the per-fader mute-button LEDs and Studio Monitor Cut's open/closed check read `_fader_volumes` directly, so a reboot with Mic/LineIn/Game genuinely closed could show all four fader LEDs green and Studio Monitor Cut actively cutting Line Out with nothing actually open.

Reproduced live twice (2026-09-14): once confirmed by physically nudging each fader through its resting position (which forces a real WebSocket-reported reading and clears it), then root-caused in code and fixed in `interfaces/goxlr.py`'s `_apply_fader_volumes()` — it now updates `self._fader_volumes[channel]` immediately alongside each `SetVolume` push, the same way `_game_level` already did. Confirmed fixed on a third reboot: correct LED colours and Cough at orange (armed, not cutting) immediately, no fader nudge needed. Commit `370fdee`. Full write-up in `ARCHITECTURE.md` §9 "Channel mute buttons".

**If you see this on an older deployment:** physically move each fader slightly (through its logical resting position) to force a live WebSocket-reported reading — this clears both symptoms immediately without a restart, and is the same workaround the amber-fader soft-pickup mechanism already relies on elsewhere.

## Issue: `goxlr-daemon.service` takes ~30 seconds to stop during a reboot or `systemctl stop`

**Not a hang, not ours to fix.** Confirmed live 2026-09-14 from the journal: `briclite.service` stops cleanly in well under a second (its own PFL WebSocket connection to the daemon is closed as part of that), then `goxlr-daemon` logs "Shutting down daemon" and goes silent for ~30 seconds before "HTTP Server Stopped." That number is the tell: `goxlr-utility` v1.2.4's daemon uses `actix-web` 4.11 for its HTTP/WebSocket server (confirmed from its `Cargo.toml`), whose default `shutdown_timeout` is exactly 30 seconds, and the daemon's own shutdown code (`main.rs`) calls `server.stop(false)` with no custom timeout override anywhere.

This matches a long-standing, still-open upstream `actix-web` limitation (GitHub `actix/actix-web#3169`): actix-web doesn't reliably tear down an already-upgraded persistent connection (like a WebSocket) promptly on shutdown, even when told to stop non-gracefully. Since briclite's own websocket is confirmed closed before the daemon starts stopping, the connection actix is waiting out is something else — most likely the GoXLR Utility web UI itself (`http://172.16.10.213:14564/`, exposed to the Lenovo broadcast laptop) if a browser tab on it was open anywhere on the network at the time, since that page holds its own persistent `/api/websocket` connection.
