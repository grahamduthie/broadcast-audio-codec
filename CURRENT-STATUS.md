# Current Investigation Status — updated 2026-09-14
...
**Lenovo (`broadcast@172.16.10.212`) setup.** `systemd-timesyncd` is enabled, active, and synchronized to `ntp.ubuntu.com`.  At desktop login, `/home/broadcast/.config/autostart/broadcast-codec.desktop` waits ten seconds and opens Firefox to `http://192.168.254.1`.  This is the direct Ethernet PSA address, not the PSA's Wi-Fi/LAN address.

**Dashboard layout and controls.** The Marlow FM logo is served from `/assets/marlowfm-logo.png` (mounted by `main.py`) at top left.  Studio Return and News sit next to the enlarged Clock.  The left side has square **PRES MIC**, **GUEST MIC**, **News**, and **PLAYER** fader cards (subtitles FADER A–D), followed by square Monitor and Input Controls cards; Mic preamp and Studio Return PFL baseline controls now live in Input Controls.  The compact card dimensions intentionally leave space below them at the Lenovo viewport.  The link control is now a state indicator plus one adjacent action: green **Link Connected** + **Disconnect**, or red **Link Disconnected** + **Connect**.

**Useful network indications, not just instantaneous counters.** `DataBroker` now retains rolling process-lifetime samples: current/5-minute-peak jitter, lost and late packets over both 60 seconds and five minutes, plus RX-watchdog outage records for five minutes.  The status is:
- **RX OUTAGE** only while recovery is actually in progress after the watchdog found no RX packets for its timeout.
- **CHECK LINK** when jitter is at least 30 ms, a lost/late packet occurred in the last 60 seconds, or a recovered watchdog outage remains in the five-minute history.
- **HEALTHY** otherwise (and **IDLE** while disconnected).

The first validated incoming ADTS packet after a reconnect calls `mark_rx_packets_resumed()`: it immediately clears the red active-outage state but preserves the recent incident as an amber Check Link warning.  This specifically fixes the earlier false-looking persistent **RX OUTAGE** while incoming audio was plainly present.  The jitter plot is deliberately hidden unless the operator presses the **GRAPH** button beside the jitter reading; it no longer opens itself on an incident.  At the last post-fix backend check, the live link reported HEALTHY, roughly 1.7 ms jitter, and zero current loss/late counts.  Note that rolling history survives automatic pipeline reconnects but resets if `briclite` itself is restarted.

Two real no-packet gaps of about 11 seconds were observed during this work; the watchdog recorded them as `No RX packets — watchdog reconnect` and rebuilt the link.  Treat a recurring Check Link/outage record as evidence to correlate with the PSA journal and the remote codec/network, rather than as proof that the dashboard itself has failed.

## Update — 2026-09-14: Deliberate resilience testing (GoXLR hotplug, full PSA reboot) — one real bug found and fixed, two stale doc claims corrected

Unrelated to the glitch investigation. Graham deliberately exercised failure scenarios ahead of handing this system to other operators elsewhere — this is exactly the kind of thing that needed live confirmation rather than reasoning from docs alone, and it paid off.

**1. GoXLR physically unplugged/replugged while the codec connection was up and passing real traffic.** Recovered largely as designed: the 5s hotplug poll triggers `_full_reconnect()`, which tears down and rebuilds the whole pipeline object and restores the network link afterward automatically if it was active — confirmed live, no manual `/api/connect` needed. As predicted, this involves a real TX gap: the codec connection dropped for a period before self-healing (remote-side session drop + our own RX watchdog, both pre-existing/expected behaviour) — Graham has decided to leave this as-is for now rather than pursue decoupling the network socket from the local-pipeline rebuild or adding a synthetic silence TX source. As also predicted, Bleep/PFL came back off (by design — a GoXLR hotplug always creates a genuinely fresh `GoXLRInterface`, which starts with PFL off via `__init__`, unlike other full-reconnect triggers that reuse the existing instance).

**2. Full PSA300 reboot (`sudo reboot`) with a connection up and PFL engaged on studio return — done twice, live.** Both times:
- The codec reconnected automatically with no manual intervention — `main INFO Restored requested codec link after service startup` in the journal, ~7s after `goxlr-daemon` started. Total outage from shutdown to reconnect was roughly 90 seconds.
- **Bleep/PFL came back on both times**, correctly matching the persisted `studio_pfl: true`. This directly contradicts `ARCHITECTURE.md`'s old "Known Limitations" item 5, which claimed PFL persistence across a full service restart was "NOT IMPLEMENTED" — that note was simply never updated after the 2026-09-11/13 restart-persistence work actually fixed it. Corrected in `ARCHITECTURE.md` this session.
- **First reboot surfaced a real bug:** all four fader mute-button LEDs showed green (open) regardless of actual fader position, and Studio Monitor Cut (Cough) showed red — actively cutting Line Out — even though Mic/LineIn/Game were all genuinely closed (`GetStatus` confirmed 0/0/0). Root-caused to `_fader_volumes` being seeded from a pre-restore hardware snapshot in `start()` and never corrected by `_apply_fader_volumes()`'s actual restore commands (full explanation in `ARCHITECTURE.md` §9 and `TROUBLESHOOTING.md`). Physically nudging the affected faders cleared it as a live workaround; the underlying bug was then fixed in `interfaces/goxlr.py` (commit `370fdee`) and deployed. **Second reboot confirmed the fix**: correct LED colours and Cough at orange (armed, not cutting) immediately at boot, no fader nudge needed.
- **goxlr-daemon takes ~30s to stop on every shutdown** (Graham noticed this watching the console). Root-caused to `actix-web`'s default 30s `shutdown_timeout` in the third-party `goxlr-daemon` v1.2.4 binary, most likely waiting out a lingering GoXLR Utility web UI browser connection — not something in this repo, not worth fixing, just worth knowing about. Full detail in `TROUBLESHOOTING.md`.

**Live state at handoff:** PSA300 is on the post-fix `goxlr.py`, connected, PFL **on** (Bleep orange, Music routed to Headphones — left engaged from testing, not something that needs undoing), Studio Monitor Cut armed and correctly showing orange (not cutting). Backups of the pre-fix file remain on the PSA300 as `interfaces/goxlr.py.bak-*` (harmless to leave, same convention as elsewhere).

## Update — 2026-09-14: Channel mute buttons coloured by fader state; Studio Monitor Cut armed colour changed to orange

Unrelated to the glitch investigation. Two small GoXLR lighting requests, both live-tested on the PSA300 and pushed to `main` (GitHub). Commits, this repo → PSA300 `/opt/briclite` local repo: `052b544`→`05bef5d` (channel mute button colours), `6998852`→`c0d2da4` (Cough armed colour). Both deployed via `scp` to `/tmp` + `sudo install` (root-owned on the PSA300) followed by the standard restart+reconnect round-trip (see `TROUBLESHOOTING.md`); neither caused a cascading outage — the second restart didn't even need the explicit `/api/connect`, the process's own restart-persistence (`ARCHITECTURE.md` §10) reconnected it automatically.

**1. Each fader's own hardware mute button (`Fader1Mute`–`Fader4Mute`, one per A–D — distinct from Bleep/Cough) now reflects both mute state and fader position**, in `briclite/interfaces/goxlr.py`:
- **Muted** (button pressed) → red (`FF0000`), full brightness, checked first so it overrides fader position entirely.
- **Unmuted + fader open** past `_MIC_OPEN_THRESHOLD` (5/255 — the same constant Studio Monitor Cut already used) → green (`00FF00`), full brightness. This applies to all four faders, not just Mic/LineIn — Studio Monitor Cut itself still only reacts to Mic/LineIn.
- **Unmuted + fader closed** → unchanged cyan (`00FFFF`), dim — the button's pre-existing native/default appearance.

Implementation notes for whoever touches this next:
- Full brightness in the button's native "off" (Unmuted) appearance requires `SetButtonOffStyle: [button, "Colour2"]`, exactly like the existing Bleep/Cough fix (`ARCHITECTURE.md` §9) — so `_set_mute_button_colour()` resends `SetButtonOffStyle` (toggling between `"Colour2"` and `"Dimmed"`) alongside `SetButtonColours` every time the open/closed state changes, not just once at start.
- **New assumption, confirmed live this session but worth re-checking if it ever looks wrong:** a `FaderXMute` button's "on" (genuinely Muted) appearance shows colour_one at full brightness regardless of `off_style` — unlike Bleep/Cough, these buttons really do enter the native Muted state, so this is a different code path from the one the earlier Bleep/Cough fix exercised. Verified live: pressing mute on an *open* fader (D) and on a *closed* fader (A/B) both showed full-bright red, matching spec.
- State tracking was generalised from Game-only (`_game_muted`, `_mic_volumes` keyed on Mic/LineIn) to all four faders (`_fader_muted`, `_fader_volumes` dicts). `on_fader_mute_changed` (used only to drive the clean-news-return audio mute) still fires only for Fader C/Game — unchanged behaviour, just re-plumbed through the generalised state.
- Pressing any fader's mute button now gives an immediate LED response on `button_down` (previously only Fader C/Game had this instant feedback; the authoritative `mute_state` patch still corrects it on release).

**2. Studio Monitor Cut's Cough LED, when armed but not actively cutting, is now the same orange (`FF8800`) Bleep shows for an engaged studio return PFL — not its own green.** One-line change: `_set_cough_colour()`'s armed-but-not-cutting branch now reuses `_PFL_COLOUR` instead of a dedicated `_MONITOR_CUT_ARMED_COLOUR` constant (removed). The actively-cutting colour (red) is untouched. Confirmed live: armed+not-cutting → orange matching Bleep's PFL colour; armed+cutting → still red.

**`ARCHITECTURE.md`'s "Cough LED" line and CURRENT-STATUS's 2026-09-13 "Current physical LED meanings" bullet below are now stale in the way described above** — updated in this same commit/session; if you find another copy of that colour list elsewhere, it needs the same correction (green → orange for "armed, Line Out live").

**Live state at handoff:** all four fader mute buttons are Unmuted; Studio Monitor Cut remains **armed** (`monitor_cut_enabled: true`, carried over from the 2026-09-13 session, not something this session engaged or needs to undo) but Line Out is currently live (no mic open) so Cough should read orange, not red. The codec is currently connected (`briclite.service` active, `/api/connect` succeeded) — this was Graham's own test session, nothing was on air. Verify current fader/mute state with `goxlr-client --status-json` (`fader_status`, `levels.volumes`) before assuming any of the above if picking this up later.

## Update — 2026-09-14: Broadcast Mix metering made independent of RTP connection status (major pipeline-lifecycle redesign)

**Request:** the Broadcast Mix meter on the dashboard should show real levels regardless of whether the codec's RTP link to the remote end is connected — it's a local GoXLR hardware mix, not something that should depend on the network link.

**Why this wasn't a small fix:** the dashboard bar height was already computed straight from telemetry (`d.tx_peak_l`/`tx_peak_r`), not gated by connection status in the JS. The real cause was upstream: `tx_meter` (the GStreamer `level` element that measures the Broadcast Mix) lived inside the same `tx_pipeline` that captured, encoded, and RTP-sent audio — and that whole pipeline was only set to `PLAYING` between `/api/connect` and `/api/disconnect`. A narrower "metering-only" tap was considered and rejected: `_full_reconnect()`'s docstring already documents (found 2026-09-09) that the GoXLR's TX (capture) and RX (playback) are two directions of the *same* physical USB device, and tearing down/reopening only one side while the other stays open reliably wedges it. Leaving TX capture running while cycling RX open/closed on every Connect/Disconnect would repeat that exact failure pattern, just far more often.

**What changed:** `PipelineController` now has four lifecycle methods instead of two (`core/pipeline_manager.py`):
- `begin_local(event_loop)` — brings the local pipeline (TX capture/metering + RX playback, plus `GoXLRInterface.start()`'s full routing/fader/colour/PFL setup) live. Called once at process startup (`main.py`'s `lifespan()`) and again after any full pipeline rebuild. Never gated on network state.
- `start(event_loop)` / `stop(event_loop)` — now mean only "network link up/down": open/close the RTP socket, start/stop the RX and playout threads, flip `connection_status` CONNECTED/DISCONNECTED. No longer touch the GStreamer pipeline state at all. `is_active` now means "network link active", not "local pipeline running".
- `full_stop(event_loop)` — the old full teardown (both TX/RX to `Gst.State.NULL` together, `glib_loop.quit()`, `interface.stop()`). Reserved for when the whole `PipelineController` object is being discarded and replaced: `_full_reconnect()` (mode change, Behringer hotplug, device-fault recovery) and process shutdown.

`_on_tx_sample()` (the TX appsink callback) now checks `is_active`/`self.sock` before transmitting or advancing RTP sequence/timestamp state — critical, since TX capture runs continuously now and must never send audio to the remote while "disconnected". It still always pulls the sample first so the appsink never backs up.

`main.py`'s `_full_reconnect()` no longer early-returns when the network is disconnected — it always rebuilds the local pipeline now (faults/hotplug/mode-changes need to be handled regardless of connection state, since the local pipeline is always running), and only restores the network link afterward (`was_active` captured before the rebuild). The bus-message device-error handler in `pipeline_manager.py` similarly no longer checks `is_active` before triggering `on_pipeline_fault` — a device-level fault must be recovered regardless of network state now. `connect_codec()`/`_restore_desired_link()` no longer construct a brand-new `PipelineController` on every connect (that would try to reopen already-open ALSA devices) — they operate on the existing, continuously-running instance.

**Follow-up fix same day:** the dashboard's Broadcast Mix *bar* height was already unconditional, but its dBFS text readout (`lbl-tx-db`) was still gated on `d.status === 'CONNECTED'` in `index.html`'s JS, blanking to "— dBFS" while disconnected even though real levels were now flowing. Fixed by dropping the `live` gate on that one readout only — Incoming Network's readout stays gated since it genuinely has no signal while disconnected.

**Live-tested and confirmed working, 2026-09-14** (nothing was on air; tested deliberately with the operator present and Console/Fader D playing music):
- Immediately after `sudo systemctl restart briclite.service`, the journal showed `SetRouter`/`SetFaderMuteFunction`/`SetFaderColours`/`SetButtonColours` commands and `GoXLR Mini ready: serial=...` — confirming `GoXLRInterface.start()` really does run at boot now, before any connect. Fader layout/routing/colours are no longer left on the stale on-disk profile pre-connect (see `TROUBLESHOOTING.md`'s updated section).
- With the codec still `DISCONNECTED`, the Broadcast Mix bar and dBFS readout tracked real music playing through Fader D (roughly -16 to -21 dBFS, moving), confirmed both via the raw telemetry websocket and visually on the live dashboard.
- `arecord -D goxlr_broadcast` from a separate shell correctly failed with "Device or resource busy" — confirming TX capture genuinely holds the ALSA device open continuously, not just producing default/stale values.
- **RX pipeline gotcha, confirmed benign:** the journal shows `RX: null→ready→paused` at boot/rebuild but *no* `paused→playing` until the moment of an actual `/api/connect` (TX reaches `playing` immediately since its ALSA source has continuous real data; RX's `appsrc` has nothing pushed to it until real RTP arrives, so GStreamer legitimately holds it in an async/preroll-pending state). This is expected GStreamer behaviour for a live pipeline whose source has no data yet, not a bug — confirmed live: pressing Connect immediately produced `RX: paused→playing` in the journal, Incoming Network started showing real Studio Return/News levels, and no errors were logged.
- Disconnect cleanly returned Incoming Network to quiet/"— dBFS" while Broadcast Mix kept metering, with no errors in the journal.

**Still not independently verified live** (lower risk, but a future session should check before assuming): RX channel-mode change while disconnected, Behringer hotplug while disconnected, and device-fault auto-recovery while disconnected — all three now behave differently per the code changes above (previously a no-op, now should actually rebuild the local pipeline), but none were deliberately exercised this session. Also unverified: whether PFL state persisted to `desired-link.json` now gets reasserted automatically at boot (plausible side effect of `interface.start()` running earlier, but PFL persistence itself wasn't touched by this change — don't assume it without checking, see `ARCHITECTURE.md` §12 roadmap item 5).

**Commits:** this repo `7f66851` (lifecycle redesign) → `48dd72b` (readout fix); PSA300 `/opt/briclite` local repo `69d183b` → `26fe3d3`. Deployed via `scp` to `/tmp` + `sudo install` (both `main.py` and `core/pipeline_manager.py` are root-owned on the PSA300, unlike the web template) followed by the standard restart+reconnect round-trip. Pre-change originals were saved alongside as `main.py.bak-20260914-114233` / `core/pipeline_manager.py.bak-20260914-114233` on the PSA300 (untracked, same convention as the existing `config.json.bak-*` files there) — harmless to leave, delete once this change has proven itself over a few real sessions.

## Update — 2026-09-13: Dashboard redesigned as a console-strip layout; GoXLR Mini confirmed to have no per-channel metering

Unrelated to the glitch investigation, but a substantial same-day session, live on the PSA300 and committed on both repos: `1277a0e` (this repo) → `8620ff2` (PSA300 `/opt/briclite`). See `ARCHITECTURE.md` §6 for the resulting layout/API reference — this entry is the *why* and the investigation trail.

**Starting point:** the old dashboard gave the jitter graph roughly half the screen, showed generic "TX Input"/"RX Output" stereo meters, offered no visibility into individual GoXLR channels, and still exposed RX Channel Routing controls (L+R/Left/Right) that are no longer needed — splitting is handled elsewhere now. Target was the Lenovo T400 operator laptop's actual browser viewport, confirmed live via `wmctrl`/`xrandr` over SSH to `broadcast@172.16.10.212`: screen 1280×800, XFCE panel 33px, Firefox window 1280×731 → usable page content roughly 1280×640.

**Live investigation of what's actually possible (important — don't re-litigate this without retesting):** the request was to show each GoXLR fader channel's audio level even before it's faded into the mix, plus a trim/gain control per channel. Tested directly on the PSA300's GoXLR Mini (with the operator's help, link briefly disconnected/reconnected around the tests, confirmed safe since nothing was on air):
- Recorded all 21 USB capture channels (`arecord -D hw:GoXLRMini,0 -c 21 ...`) while cycling Mic, LineIn (Behringer), and Console (music player) one at a time. Only channels 0-1 (Broadcast Mix, a post-fader sum of everything routed there) and 2-3 ("Chat Mic", isolated to Mic only) carry any signal; nothing else responds to any of the three sources. Chat Mic is also gated to silence whenever the Mic fader itself reads near 0 — it is *not* a pre-fader tap, despite what `GOXLR-MINI-LINUX.md` §3 might suggest.
- Watched the GoXLR daemon's own control WebSocket (`ws://localhost:14564/api/websocket`) for several seconds while the operator spoke into the mic: zero spontaneous meter/level events of any kind.
- Tried the daemon's native `GetMicLevel` IPC command directly (`{"GetMicLevel": serial}` over `/tmp/goxlr.socket`): returns a live-looking float, but it's a hardcoded `-72.2` stub on this Mini variant regardless of real mic input or fader position — confirmed by raising the fader and talking continuously with no change.
- **Conclusion: there is no live per-channel audio-level metering anywhere in the GoXLR Mini's software/API for Mic, LineIn, or Console.** The only real, always-accurate levels on the whole dashboard are the two signals briclite already taps itself in its own RX decode pipeline *before* they reach the GoXLR at all — these are just the old RX-L/RX-R meter, relabelled **Studio Return** (Music bus, L) and **News** (Game bus, R).

**Trim/gain control — what shipped vs. what was tried and reverted:**
- **Mic** keeps a real control: the GoXLR's hardware preamp gain (`SetMicrophoneGain`, IPC-confirmed command name, 0-72 range per `goxlr-client --help`), independent of the fader. This is genuinely a pre-fader gain stage and is exposed on the Mic strip.
- **LineIn/Console/Game have no hardware gain stage at all** — only the fader (`SetVolume`) exists for them. A software "trim" was implemented anyway: intercepting the GoXLR WebSocket's fader-position patches and rescaling `SetVolume` against the raw physical reading (e.g. trim=80% ⇒ pushing the fader fully up only sends 204/255). First implementation queued one `SetVolume` IPC call per WebSocket patch and, under a real drag, backlogged into a multi-second flood of stale corrections well after the operator had stopped moving the fader — visibly disturbing its LEDs. Fixed with a coalescing per-channel worker (latest-value-wins, at most one IPC call in flight) that resolved the flood. **Then reverted anyway**, once fixed: the GoXLR's fader LEDs are driven by that exact `SetVolume` value, so *any* working trim implementation necessarily makes a trimmed fader's LEDs show less than full illumination at the physical top of its travel (confirmed live: Fader B pushed fully up with trim=80% showed only ~80% of its LEDs lit). The operator judged this not worth the confusion. **If trim for these three channels is ever revisited, this LED/gain coupling is the fundamental constraint to design around, not a bug to fix** — e.g. a "suggested trim" number shown only on the dashboard, with no actual hardware rescale, would avoid it.

**Other UI decisions worth knowing about:** the old digital/analogue meter-style toggle and SVG VU-needle gauges were dropped in favour of one consistent digital "LED bar" look (the analogue gauges only ever applied uniformly to TX/RX, and would have looked inconsistent applied to only 2 of 8 new modules). The jitter graph is now collapsed by default and opens only when the operator presses the GRAPH button; it never auto-expands.

**Possible follow-up, not started:** a genuine per-channel audio tap for LineIn/Console (the only two with truly no software or hardware level data at all) would require a hardware change — e.g. physically wiring their signal into a spare capture channel on the Behringer or another interface. Not attempted this session; flag to the user before spending time on it, since it needs hands-on-hardware access.
...
- **Current physical LED meanings**, all via `SetButtonColours`/`SetFaderColours` in `goxlr.py`: Bleep — cyan (`00FFFF`) = PFL off, orange (`FF8800`) = PFL on. Cough — cyan = Studio Monitor Cut disarmed, orange (`FF8800`, same as Bleep's PFL-on colour, changed from green 2026-09-14) = armed and Line Out live, red (`FF0000`) = armed and actively cutting Line Out. Faders A–D — blue-ish (low, `00FFFF` normal / `FFB000` amber pending-pickup) fading to red (`FF0000`, fixed top anchor). Each fader's own mute button (`Fader1Mute`–`Fader4Mute`, added 2026-09-14) — red full-bright when muted, green full-bright when unmuted and open past the Line Out mute threshold, cyan dim when unmuted and closed; see the 2026-09-14 entry above for details.
