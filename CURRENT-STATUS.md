...
**Side effect, not yet independently verified live:** since `GoXLRInterface.start()` now runs at every process boot (inside `begin_local()`) instead of only on first connect, the correct fader layout/routing/colours should now be visible within a second or two of `briclite.service` starting — before anyone presses Connect. See `TROUBLESHOOTING.md`'s "GoXLR faders/colours look wrong after a reboot" section, updated to match. Also plausible but unverified: PFL state persisted to `desired-link.json` may now get reasserted automatically at boot too, which would be a partial fix for the long-standing "PFL comes back off after a deploy" issue (`ARCHITECTURE.md` §12 roadmap item 5) — don't assume this without checking live, since PFL persistence itself wasn't touched by this change.

**Deploy note:** this is a substantially larger change than this repo's usual dashboard/routing tweaks — it touches the core connect/disconnect/reconnect/fault-recovery state machine end to end. Test the full matrix live before trusting it in production: boot behaviour (GoXLR comes up correctly pre-connect), Connect/Disconnect (Broadcast Mix keeps metering after Disconnect, Incoming Network correctly goes quiet), RX mode change and Behringer hotplug while disconnected (previously a no-op, now should actually rebuild), and that nothing gets transmitted to the remote while disconnected.
